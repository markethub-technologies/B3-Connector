  ...
  class MdPublishWorker final {
   public:
    ...
    MdPublishWorker(uint32_t shardId, MdSnapshotMapper &mapper, publishing::IPublishSink &sink)
        : shardId_(shardId), mapper_(mapper), sink_(sink) {}
   ...
   private:
    ...
    void run() noexcept {
      ...
      auto publish_one = [&](const OrdersSnapshot &s) {
        ...
        // TODO: cuando esté el map instrumentId->symbol, armás {SYMBOL}-D acá
        static constexpr char kTopic[] = "B3-D";
        static constexpr uint8_t kTopicLen = static_cast<uint8_t>(sizeof(kTopic) - 1);

        ev.topicLen = kTopicLen;
        std::memcpy(ev.topic, kTopic, kTopicLen);
