...
namespace details {
    ...
    struct HasExponent
    {
        ...
        enum
        {
            value = sizeof(test<T>(ONIXS_B3_UMDF_MD_NULLPTR)) == sizeof(char)
        };
    };
    ...
    {
        template<typename U, void (U::*)(void*) const ONIXS_B3_UMDF_MD_NOTHROW> struct SFINAE {};
    ...
    /// \private
    template<typename T>
    struct HasBitsMember
    {
        template<typename U> struct SFINAE {};
        template<typename U> static char test(SFINAE<typename U::Bits>*);
        template<typename U> static int test(...);
        enum { value = sizeof(test<T>(ONIXS_B3_UMDF_MD_NULLPTR)) == sizeof(char) };
    };
}
...
///\private
template <typename Set, typename NullValue>
typename EnableIf<details::HasBitsMember<Set>::value, bool>::type
operator != (NullValue null, Set set) ONIXS_B3_UMDF_MD_NOTHROW
{
    ONIXS_B3_UMDF_MD_STATIC_ASSERT((sizeof(typename Set::Bits) == sizeof(typename NullValue::Value)));
    ONIXS_B3_UMDF_MD_STATIC_ASSERT((details::IsSameSignedness<typename Set::Bits, typename NullValue::Value>::value));

    return set.bits() != null;
}
...
typename EnableIf<details::IsSameSignedness<To, From>::value, To>::type
    numericCast(From from) ONIXS_B3_UMDF_MD_NOTHROW
{
    const To to =
        static_cast<To>(from);
